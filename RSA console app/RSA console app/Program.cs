...
namespace RSA_console_app
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (pt > keyPair.Item1.n)
                throw new Exception();
            Console.WriteLine("before:  " + pt);

            BigInteger ct = BigInteger.ModPow(pt, keyPair.Item1.e, keyPair.Item1.n);
            //Console.WriteLine("Encoded:  " + ct);
            Console.WriteLine(ASCIIEncoding.UTF8.GetString(ct.ToByteArray()));

            BigInteger dc = BigInteger.ModPow(ct, keyPair.Item2.d, keyPair.Item2.n);
